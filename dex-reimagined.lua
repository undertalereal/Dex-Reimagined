--[[
	...
	Made by soulopsundertale
	
	Dex:Reimagined is a revival of Moon's Dex, made to fulfill Moon's Dex prophecy.
]]
...
-- Common Locals
local Main,Lib,Apps,Settings -- Main Containers
local Explorer, Properties, ScriptViewer, ModelViewer, DecalPreview, AnimationViewer, Notebook -- Major Apps
local API,RMD,env,service,plr,create,createSimple -- Main Locals
...
local function initAfterMain()
	...
	DecalPreview = Apps.DecalPreview
...
local function main()
	...
	Explorer.ShowRightClick = function(MousePos)
		...
		if presentClasses["Decal"] then
			context:AddRegistered("DECAL_PREVIEW")
		end
		...
		if presentClasses["Animation"] then
			...
			context:AddRegistered("ANIMATION_PREVIEW")
		end
	...
	Explorer.InitRightClick = function()
		...
		local decalIconInd = (RMD.Classes["Decal"] and tonumber(RMD.Classes["Decal"].ExplorerImageIndex)) or 54
		local animationIconInd = (RMD.Classes["Animation"] and tonumber(RMD.Classes["Animation"].ExplorerImageIndex)) or 54

		context:Register("DECAL_PREVIEW",{Name = "Decal Preview", IconMap = Explorer.ClassIcons, Icon = decalIconInd, OnClick = function()
			local sList = selection.List
			local isa = game.IsA

			if #sList == 1 then
				if isa(sList[1].Obj,"Decal") or isa(sList[1].Obj,"Texture") then
					DecalPreview.ViewDecal(sList[1].Obj)
					return
				end
			end
		end})

		context:Register("ANIMATION_PREVIEW",{Name = "Animation Preview", IconMap = Explorer.ClassIcons, Icon = animationIconInd, OnClick = function()
			local sList = selection.List
			local isa = game.IsA

			if #sList == 1 then
				if isa(sList[1].Obj,"Animation") then
					AnimationViewer.ViewAnimation(sList[1].Obj)
					return
				end
			end
		end})
...
["DecalPreview"] = function()
--[[
	Decal Preview App Module
]]
...
-- handles rbxassetid://, roblox.com/asset urls, or a bare id
local function extractAssetId(textureStr)
	if not textureStr then return nil end
	local id = textureStr:match("rbxassetid://(%d+)") or textureStr:match("[?&]id=(%d+)") or textureStr:match("^(%d+)$")
	return id
end

local function formatDate(isoStr)
	if not isoStr then return "Unknown" end
...
local function main()
	...
	local window, image, pathLabel
	local nameLabel, creatorLabel, idLabel, createdLabel
	local currentFetchId = 0
	local currentName, currentOwner, currentId, currentCreated = "","","",""

	local function setInfoText(name, creator, id, created)
		currentName, currentOwner, currentId, currentCreated = name or "", creator or "", id or "", created or ""
		nameLabel.Text = "Name: " .. (name or "...")
		creatorLabel.Text = "Owner: " .. (creator or "...")
		idLabel.Text = "Asset ID: " .. (id or "...")
		createdLabel.Text = "Created: " .. (created or "...")
	end

	DecalPreview.ViewDecal = function(inst)
		if not (inst and (inst:IsA("Decal") or inst:IsA("Texture"))) then return end

		window:SetTitle(inst.Name.." - Decal Preview")
		window:Show()

		local id = extractAssetId(inst.Texture)
		if not id then
			setInfoText(inst.Name, "Unknown", "N/A", "Unknown")
			image.Image = ""
			pathLabel.Gui.Text = "path: " .. getPath(inst)
			return
		end

		image.Image = "rbxassetid://" .. id
		pathLabel.Gui.Text = "path: " .. getPath(inst)
		setInfoText(inst.Name, "Loading...", id, "Loading...")

		-- avoid race if user clicks another decal before this resolves
		currentFetchId = currentFetchId + 1
		local myFetchId = currentFetchId

		task.spawn(function()
			local ok, result = pcall(function()
				local raw = game:HttpGet("https://economy.roblox.com/v2/assets/" .. id .. "/details")
				return service.HttpService:JSONDecode(raw)
			end)

			if myFetchId ~= currentFetchId then return end -- a newer decal was opened while this was loading

			if ok and result then
				local creatorName = result.Creator and result.Creator.Name or "Unknown"
				setInfoText(result.Name or inst.Name, creatorName, id, formatDate(result.Created))
			else
				setInfoText(inst.Name, "Failed to fetch", id, "Failed to fetch")
			end
		end)
	end

	DecalPreview.Init = function()
		window = Lib.Window.new()
		window:SetTitle("Decal Preview")
		window:Resize(280,340)
		DecalPreview.Window = window

		image = Instance.new("ImageLabel")
		image.Parent = window.GuiElems.Content
		image.BackgroundColor3 = Settings.Theme.Main2
		image.BorderSizePixel = 0
		image.Position = UDim2.new(0,0,0,0)
		image.Size = UDim2.new(1,0,0,180)

		local infoHolder = Instance.new("Frame")
		infoHolder.Parent = window.GuiElems.Content
		infoHolder.BackgroundTransparency = 1
		infoHolder.Position = UDim2.new(0,4,0,184)
		infoHolder.Size = UDim2.new(1,-8,1,-200)

		local listLayout = Instance.new("UIListLayout")
		listLayout.Parent = infoHolder
		listLayout.Padding = UDim.new(0,2)

		local function makeInfoRow(getValue)
			...
			lbl.Gui.Parent = row
			lbl.Gui.Size = UDim2.new(1,-20,1,0)
			lbl.Gui.TextXAlignment = Enum.TextXAlignment.Left
			lbl.Gui.TextSize = 13

			local copyButton = Instance.new("ImageButton")
			copyButton.Parent = row
			copyButton.AnchorPoint = Vector2.new(1,0.5)
			copyButton.Position = UDim2.new(1,0,0.5,0)
			copyButton.Size = UDim2.new(0,14,0,14)
			copyButton.BackgroundTransparency = 1
			copyButton.ImageTransparency = 0.4
			Explorer.MiscIcons:DisplayByKey(copyButton, "Copy")

			copyButton.MouseButton1Click:Connect(function()
				if env.setclipboard then
					env.setclipboard(getValue())
				end
			end)
		...
		nameLabel = makeInfoRow(function() return currentName end)
		creatorLabel = makeInfoRow(function() return currentOwner end)
		idLabel = makeInfoRow(function() return currentId end)
		createdLabel = makeInfoRow(function() return currentCreated end)
...
-- Common Locals
local Main,Lib,Apps,Settings -- Main Containers
local Explorer, Properties, ScriptViewer, ModelViewer, DecalPreview, Notebook -- Major Apps
local API,RMD,env,service,plr,create,createSimple -- Main Locals
...
local function extractAssetId(str)
	...
	local id = str:match("rbxassetid://(%d+)") or str:match("[?&]%a+=(%d+)") or str:match("^(%d+)$")
	return id
end
...
local function main()
	...
	local window, viewport, worldModel, camera, pathLabel
	local nameLabel, creatorLabel, idLabel, createdLabel
	local currentFetchId = 0
	local currentName, currentOwner, currentId, currentCreated = "","","",""

	local currentClone, currentTrack
	local rotating, lastX = false, 0
	...
	local function updateCamera()
		...
		local offset = CFrame.Angles(0, orbitYaw, 0) * Vector3.new(0, camDistance * 0.4, camDistance)
		camera.CFrame = CFrame.lookAt(camTarget + offset, camTarget)
	end
	...
	AnimationViewer.ViewAnimation = function(inst)
		if not (inst and inst:IsA("Animation")) then return end

		window:SetTitle(inst.Name.." - Animation Preview")
		window:Show()
		pathLabel.Gui.Text = "path: " .. getPath(inst)

		cleanup()

		-- Clone the player's own character to act as the preview rig
		local char = plr.Character
		if not char then
			setInfoText(inst.Name, "No character to preview on", "N/A", "N/A")
			return
		end

		-- some games set Archivable=false to block cloning, force it back on
		...
		if not ok or not clone then
			setInfoText(inst.Name, "Failed to clone character", "N/A", "N/A")
			return
		end
		...
		if not humanoid or not rootPart then
			setInfoText(inst.Name, "Rig has no Humanoid", "N/A", "N/A")
			return
		end

		-- Freeze the rig in place so it doesn't fall/walk off during preview
		...
		animObj.AnimationId = inst.AnimationId
		...
		local id = extractAssetId(inst.AnimationId)
		if not id then
			setInfoText(inst.Name, "Unknown", "N/A", "Unknown")
			return
		end
		...
		task.spawn(function()
			...
			if myFetchId ~= currentFetchId then return end
	...
	AnimationViewer.Init = function()
		window = Lib.Window.new()
		window:SetTitle("Animation Preview")
		window:Resize(300,360)
		AnimationViewer.Window = window

		viewport = Instance.new("ViewportFrame")
		viewport.Parent = window.GuiElems.Content
		viewport.BackgroundColor3 = Settings.Theme.Main2
		...
		viewport.InputChanged:Connect(function(input)
			if rotating and input.UserInputType == Enum.UserInputType.MouseMovement then
				...
				lastX = input.Position.X
				orbitYaw = orbitYaw - math.rad(delta * 0.5)
				updateCamera()
			end
		end)
		...
		viewport.MouseWheelForward:Connect(function()
			camDistance = math.clamp(camDistance - 1, 2, 60)
			updateCamera()
		end)
		viewport.MouseWheelBackward:Connect(function()
			camDistance = math.clamp(camDistance + 1, 2, 60)
			updateCamera()
		end)

		local divider = Instance.new("Frame")
		divider.Parent = window.GuiElems.Content
		divider.BackgroundColor3 = Settings.Theme.Outline1
		...
		infoHolder.Parent = window.GuiElems.Content
		infoHolder.BackgroundTransparency = 1
		infoHolder.Position = UDim2.new(0,4,0,208)
		infoHolder.Size = UDim2.new(1,-8,1,-224)

		local listLayout = Instance.new("UIListLayout")
		listLayout.Parent = infoHolder
		listLayout.Padding = UDim.new(0,4)

		-- one field (label + copy icon), half-width so two fit per row
		local function makeField(parent, widthScale, getValue)
			local holder = Instance.new("Frame")
			holder.Parent = parent
			holder.BackgroundTransparency = 1
			holder.Size = UDim2.new(widthScale,0,1,0)

			local lbl = Lib.Label.new()
			lbl.Gui.Parent = holder
			lbl.Gui.Size = UDim2.new(1,-18,1,0)
			lbl.Gui.TextXAlignment = Enum.TextXAlignment.Left
			lbl.Gui.TextSize = 13
		...
		local function makePairRow(getValue1, getValue2)
			local row = Instance.new("Frame")
			row.Parent = infoHolder
			row.BackgroundTransparency = 1
			row.Size = UDim2.new(1,0,0,18)

			local left = makeField(row, 0.5, getValue1)
			local right = makeField(row, 0.5, getValue2)
			right.Parent.Position = UDim2.new(0.5,4,0,0)
			right.Parent.Size = UDim2.new(0.5,-4,1,0)
			left.Parent.Size = UDim2.new(0.5,-4,1,0)

			return left, right
		end

		nameLabel, idLabel = makePairRow(
			function() return currentName end,
			function() return currentId end
		)
		creatorLabel, createdLabel = makePairRow(
			function() return currentOwner end,
			function() return currentCreated end
		)
...
local function main()
	...
	ScriptViewer.Init = function()
		...
		local searchBox = Instance.new("TextBox",window.GuiElems.Content)
		searchBox.BackgroundColor3 = Color3.new(0.13,0.13,0.13)
		searchBox.BorderSizePixel = 0
		searchBox.Position = UDim2.new(0,4,0,22)
		searchBox.Size = UDim2.new(1,-84,0,16)
		searchBox.Text = ""
		searchBox.PlaceholderText = "Find in script..."
		searchBox.TextColor3 = Color3.new(1,1,1)
		searchBox.TextSize = 13
		searchBox.ClearTextOnFocus = false
		searchBox.TextXAlignment = Enum.TextXAlignment.Left

		local matchLabel = Instance.new("TextLabel",window.GuiElems.Content)
		matchLabel.BackgroundTransparency = 1
		matchLabel.Position = UDim2.new(1,-80,0,22)
		matchLabel.Size = UDim2.new(0,30,0,16)
		matchLabel.Text = ""
		matchLabel.TextColor3 = Color3.new(0.6,0.6,0.6)
		matchLabel.TextSize = 12

		local prevBtn = Instance.new("TextButton",window.GuiElems.Content)
		prevBtn.BackgroundColor3 = Color3.new(0.13,0.13,0.13)
		prevBtn.BorderSizePixel = 0
		prevBtn.Position = UDim2.new(1,-48,0,22)
		prevBtn.Size = UDim2.new(0,22,0,16)
		prevBtn.Text = "▲"
		prevBtn.TextColor3 = Color3.new(1,1,1)
		prevBtn.TextSize = 12

		local nextBtn = Instance.new("TextButton",window.GuiElems.Content)
		nextBtn.BackgroundColor3 = Color3.new(0.13,0.13,0.13)
		nextBtn.BorderSizePixel = 0
		nextBtn.Position = UDim2.new(1,-24,0,22)
		nextBtn.Size = UDim2.new(0,22,0,16)
		nextBtn.Text = "▼"
		...
		local function jumpToMatch(i)
			local m = searchMatches[i]
			if not m then
				matchLabel.Text = "0/0"
				clearHighlight()
				return
			...
			-- CodeFrame rows/cols are 0-indexed, string.find is 1-indexed
			codeFrame.SelectionRange = {{m.startIdx-1, m.row-1},{m.endIdx, m.row-1}}

			local targetView = math.max(0, (m.row-1) - 3) -- a little context above the match
			codeFrame.ScrollV:ScrollTo(targetView)
			codeFrame:Refresh()

			matchLabel.Text = i.."/"..#searchMatches
		end

		local function runSearch()
			local term = searchBox.Text
			table.clear(searchMatches)
			searchIndex = 0
			...
			for i = 1,#lines do
				local lowerLine = lines[i]:lower()
				local searchFrom = 1
				while true do
					local s,e = lowerLine:find(lowerTerm, searchFrom, true)
					if not s then break end
					table.insert(searchMatches, {row = i, startIdx = s, endIdx = e})
					searchFrom = e + 1
				end
			end
		...
		searchBox.FocusLost:Connect(function(enterPressed)
			if enterPressed and #searchMatches > 0 then
				searchIndex = searchIndex % #searchMatches + 1
		...
		prevBtn.MouseButton1Click:Connect(function()
			if #searchMatches == 0 then return end
			searchIndex = (searchIndex - 2) % #searchMatches + 1
			jumpToMatch(searchIndex)
		end)
...
Main = (function()
	...
	Main.ModuleList = {"Explorer","Properties","ScriptViewer","Console","SaveInstance","ModelViewer","DecalPreview","AnimationViewer","SettingsWindow"}
	Main.Elevated = false
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{5,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=4,Name="Title",Parent={3},Position=UDim2.new(0,-260,0,20),Size=UDim2.new(0,260,0,40),Text="Dex:Reimagined",TextColor3=Color3.new(1,1,1),TextSize=32,TextTransparency=1,TextXAlignment=0,}},
			{6,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Desc",Parent={3},Position=UDim2.new(0,-270,0,60),Size=UDim2.new(0,220,0,25),Text="Ultimate Debugging Suite",TextColor3=Color3.new(1,1,1),TextSize=18,TextTransparency=1,}},
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="Made by soulopsundertale",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
		...
		tweenS:Create(descText,ti,{Position = UDim2.new(0,20,0,64), TextTransparency = 0}):Play()
	...
	Main.SetMainGuiOpen = function(val)
		...
		Main.MainGui.OpenButton.Text = val and "Close" or "Dex:Reimagined"
	...
	Main.CreateMainGui = function()
		...
		Main.CreateApp({Name = "Decal Viewer", IconMap = Main.LargeIcons, Icon = "Watcher", Window = DecalPreview.Window})

		Main.CreateApp({Name = "Animation Viewer", IconMap = Main.LargeIcons, Icon = "ScriptEdit", Window = AnimationViewer.Window})
